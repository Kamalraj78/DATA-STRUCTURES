...
struct lnode{
    int data; //its value
    lnode* next; // we can't put int* next becoz it points not only to next node's value,it points to entire node
};


typedef struct lnode node;//instead of using struct lnode we can use node while node creation everywhere


class LinkedList{

    node* head = NULL;//head node creation
public:
